        ...
        <div className="bg-white rounded-lg shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <FormField label="Correo Electrónico" error={error}>
              ...
              <Input type="email" autoComplete='username' value={email} placeholder="usuario@ejemplo.com"
                className='pl-10 pr-4' onChange={(e) => { setEmail(e.target.value); setError(''); }}
              />
            </FormField>
            <FormField label="Contraseña" error={error}>
              <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <Input type={showPassword ? 'text' : 'password'} value={password} placeholder="••••••••"
                className="pl-10 border-gray-300" autoComplete='current-password'
